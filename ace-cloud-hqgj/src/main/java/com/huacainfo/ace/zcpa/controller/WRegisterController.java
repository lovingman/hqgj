 ...
 * @Date 2019/8/5 17:27
 */
@Api(value = "/www/reg", tags = "人员注册相关接口")
@RestController
...
public class WRegisterController extends BizController {
    ...
    public ResponseDTO register(BaseVolunteerVo data) throws Exception {
     ...
     * @return 反馈结果 ResponseDTO
     */
    @ApiOperation(value = "/isLogin", notes = "注册接口")
     ...
     * @return 反馈结果 ResponseDTO
     */
    @ApiOperation(value = "/getAreaCode", notes = "获取当前登录用户的AreaCode")
    @GetMapping(value = "/getAreaCode", produces = "application/json;charset=UTF-8")
    public ResponseDTO getAreaCode() {
        //登录信息
        UserProp user = this.getCurUserProp();
        if (user == null) {
            return new ResponseDTO(ResultCode.FAIL, "尚未登录");
        }

        Map<String, Object> rst = registerService.getAreaCode(user.getUserId());
        return new ResponseDTO(ResultCode.SUCCESS, "获取成功", rst);
    }
}
