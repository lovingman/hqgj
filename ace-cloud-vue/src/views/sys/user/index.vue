  ...
  <div class="app-container">
    <el-card shadow="never">
      <el-row :gutter="20">
        <el-col :span="2">
          <el-button-group>
            <el-button @click="create" type="primary" style="border-radius:5px;" v-if="userBtn['/portal/user/create']">
              {{userBtn['/portal/user/create']}}</el-button>
          </el-button-group>
        </el-col>
        <el-col :span="5">
          <el-radio-group @change="toggleTab" size="medium" v-model="query.status">
            <el-radio-button :key="item.id" :label="item.id" v-for="item in statusData">{{item.name}}</el-radio-button>
          </el-radio-group>
        </el-col>
        <el-col :span="5">
          <el-cascader :options="areaCodeOptions" :props="areaCodeProps" change-on-select clearable filterable
            placeholder="请选择行政区划" style="width:100%" v-model="areaCodes" />
        </el-col>

        <el-col :span="12">
          <el-input @change="toggleChange" class="input-with-select" placeholder="请输入用户名称" v-model="query.userName">
            <el-select :loading="loading" :remote-method="corpQuery" class="type" clearable filterable
              placeholder="请选择所在组织" remote reserve-keyword slot="prepend" v-model="query.corpId">
              <el-option :key="item.value" :label="item.label" :value="item.value" v-for="item in corps"></el-option>
            </el-select>

            <el-button :loading="loading" @click="handleQuery" icon="el-icon-search" slot="append"></el-button>
          </el-input>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="24">
          <el-table :data="rows" element-loading-spinner="el-icon-loading" element-loading-text="加载中" ref="filterTable"
            ...
            <el-table-column align="right" label="操作" prop="opt">
              <template slot-scope="props">
                <el-button @click="assignRoles(props.row)" type="text" v-if="userBtn['/portal/user/assignRoles']">
                  {{userBtn['/portal/user/assignRoles']}}</el-button>
                <span class="strightline" v-if="userBtn['/portal/user/assignRoles']">|</span>

                <el-button @click="initPwd(props.row)" type="text" v-if="userBtn['/portal/user/initPwd']">
                  {{userBtn['/portal/user/initPwd']}}</el-button>
                <span class="strightline" v-if="userBtn['/portal/user/initPwd']">|</span>

                <el-button @click="update(props.row)" type="text" v-if="userBtn['/portal/user/update']">编辑</el-button>
                <span class="strightline" v-if="userBtn['/portal/user/update']">|</span>

                <el-button @click="deleteById(props.row)" type="text" v-if="userBtn['/portal/user/deleteById']">删除
                </el-button>
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="20">
          <el-pagination :current-page="currentPage" :page-size="pagesize" :total="total"
            @current-change="handleCurrentChange" @size-change="handleSizeChange" background layout="prev, pager, next">
          </el-pagination>
        </el-col>
      </el-row>
    </el-card>
    ...
    <el-dialog :visible.sync="dialogPreviewVisible" title="详细" width="80%">
      <el-container>
        <el-header style="height:1.5rem"></el-header>
        <el-main element-loading-spinner="el-icon-loading" element-loading-text="加载中" v-loading="previewLoading">
          <el-row :gutter="40">
            <el-col :span="24">
    ...
    <el-dialog :visible.sync="dialogInitPwdVisible" title="密码重置" width="50%">
      <el-container element-loading-spinner="el-icon-loading" element-loading-text="请求中" v-loading="initPwdLoading">
      ...
      <div class="dialog-footer" slot="footer">
        ...
        <el-button :loading="initPwdLoading" @click="submitForm('ruleForm')" type="primary" style="border-radius:5px;">
          保存</el-button>
      </div>
    </el-dialog>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      deleteById(data) {
        ...
        getMyRoles(data.userId).then(res => {
          if (res.status == 1) {
            if (res.data.length == 0) {
              this.$confirm("删除" + data.name + ", 是否继续?", "提示", {
                ...
                .then(() => {
                  this.loading = true;
                  deleteById(data.userId).then(response => {
                    this.loading = false;
                    this.handlePageList();
                  });
                })
      ...
      preview(data) {
        ...
        getById(data.userId).then(response => {
