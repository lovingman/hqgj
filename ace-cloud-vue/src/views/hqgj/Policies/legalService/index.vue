  ...
  <div class="main-box">
    ...
    <div class="table-box">
      <el-table :data="tableData" v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="名称" prop="title" sortable min-width="220" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column label="上传时间" prop="createDate" width="180" sortable></el-table-column>
...
export default {
  ...
  methods: {
    ...
    edit(index, data) {
      this.$router.push({
        path: "/hqgj/Policies/legalService/edit",
        query: { id: data.id }
      });
    },
    //详情
    preview(index, data) {
      this.$router.push({
        path: "/hqgj/Policies/legalService/details",
        query: { id: data.id }
      });
    }
  }
