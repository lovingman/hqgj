<template>
  <div class="photo">
    <el-row>
      <el-form :model="photoForm" :inline="true" ref="photoForm" :rules="photoRules" label-width="120px"
        class="innerOrgRuleForm">
        <el-form-item label="封面：" prop="imgArr" :class="{isError:hideError}">
          <el-upload :class="{hides:hideUpload}" action="fakeaction" list-type="picture-card" :limit="limitImgCount"
            :file-list="photoForm.imgArr" :http-request="uploadServerEdit" :on-success="unitPictrueUploadSuccess"
            :on-preview="unitPictureCardPreview" :before-upload="beforeAvatarUpload" :on-remove="unitPictrueRemove">
