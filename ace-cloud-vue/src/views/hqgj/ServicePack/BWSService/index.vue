  ...
  <div class="container">
    <div class="table">
      ...
      <el-table :data="list" class="table" max-height="475" ref="multipleTable" v-loading="loading">
        ...
        <el-table-column label="注册企业名称" prop="companyName" :show-overflow-tooltip="true"  width="125"></el-table-column>
    ...
    <el-dialog :visible.sync="progressVisible" title="办理进度标记" width="60%">
          ...
          <template slot="tag" slot-scope="{ item }">
            ...
            <div v-if="item.code!= '0'">
              <el-radio
                ...
                @click.native.prevent="clickitem(1,item.code)"
                label="1"
              >
...
export default {
  ...
  methods: {
    ...
    examine(index, data) {
      this.$router.push({
        path: "/hqgj/ServicePack/BWSService/Examine",
        query: { id: data.id, word: "examine" }
      });
    },
    preview(index, data) {
      this.$router.push({
        path: "/hqgj/ServicePack/BWSService/detail",
        query: { id: data.id, word: "preview" }
      });
    },
    progress(index, data) {
    ...
    clickitem(e, x) {
      console.log(e, x);
      e === this.items[x].checked
        ? (this.items[x].checked = "")
        : (this.items[x].checked = e);
      console.log(e, x);
      if (e == "1") {
        this.byid.tab = x;
        this.dictQuery();
